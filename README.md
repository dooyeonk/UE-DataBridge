...
Examples/Scripts/                       # VCS 통합 wrapper 예제 (Win/macOS/Linux + LFS/Perforce)
```
...
### 6. **VCS 락 처리는 wrapper에 위임**
플러그인이 `git lfs lock` / `p4 edit`을 직접 호출하지 않음. 이유:
- VCS 종류가 다양 (Git, Git LFS, Perforce, SVN, Plastic SCM)
- 사용자 환경별 인증/설정 다름
- 플러그인이 외부 명령 실행은 보안/의존성 안티패턴

대신 `Examples/Scripts/`에 환경별 예제 wrapper 제공. 사용자가 자기 환경에 맞게 수정.

### 7. **Process-level 캐시 (PIE persistence)**
`bEnablePIECache=true`이면 캐시를 process-level static에 보관. PIE 재시작해도 살아 있어서 같은 시트를 매번 다시 받지 않음. 개발 중 빠른 이터레이션용.

### 8. **환경 분기는 Source 단위 URL 맵**
...
- [Examples/Scripts/README.md](Examples/Scripts/README.md) — Wrapper script 사용법 (Git LFS / Perforce)
